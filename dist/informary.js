...
* @class CashMoney
* @constructor
*/const libLinkedList=require("./LinkedList.js");class CashMoney{constructor(){// The map of node objects by hash because Reasons.
...
this._HashMap[pHash].Datum=pData;return this._HashMap[pHash].Datum;}let tmpNode=this._List.push(pData,pHash);this._HashMap[tmpNode.Hash]=tmpNode;// Automatically prune if over length, but only prune this nodes worth.
...
this.pruneBasedOnLength(fComplete,tmpRemovedRecords);},tmpRemovedRecords);}// Read a datum by hash from the cache
read(pHash){if(!this._HashMap.hasOwnProperty(pHash))return false;return this._HashMap[pHash].Datum;}// Get a low level node (including metadata statistics) by hash from the cache
getNode(pHash){if(!this._HashMap.hasOwnProperty(pHash))return false;return this._HashMap[pHash];}}module.exports=CashMoney;},{"./LinkedList.js":3}],2:[function(require,module,exports){/**
...
let tmpHash=typeof pHash!='undefined'?pHash:"NODE[".concat(this.totalNodes,"]");let tmpNode=new libLinkedListNode();tmpNode.Hash=tmpHash;tmpNode.Datum=pDatum;return tmpNode;}// Add a node to the end (right of tail) of the list.
...
var libInformary=require('./Informary.js');if(typeof window==='object'){window.Informary=libInformary;}module.exports=libInformary;},{"./Informary.js":14}],13:[function(require,module,exports){/**
* @license MIT
* @author <steven@velozo.com>
*/ /**
* Informary Logging
*
* @class InformaryLog
*/class InformaryLog{constructor(pSettings){this._Settings=pSettings;}writeConsole(pLevel,pMessage,pObject){// Write the message
console.log('['+pLevel+'] ('+this._Settings.Form+') '+pMessage);// Write out the object if it is passed in
if(typeof pObject!=='undefined'){console.log(JSON.stringify(pObject,null,4));}}trace(pMessage,pObject){this.writeConsole('TRACE',pMessage,pObject);}debug(pMessage,pObject){this.writeConsole('DEBUG',pMessage,pObject);}info(pMessage,pObject){this.writeConsole('INFO',pMessage,pObject);}warning(pMessage,pObject){this.writeConsole('WARNING',pMessage,pObject);}error(pMessage,pObject){this.writeConsole('ERROR',pMessage,pObject);}// Log the current date and time, well formatted (with Moment-Timezone)
logTime(pMessage){let tmpMessage=typeof pMessage!=='undefined'?pMessage:'Time';let tmpDate=new Date();this.info(tmpMessage+': '+tmpDate.toString());}// Get a timestamp
...
logTimeDelta(pTimeStamp,pMessage){let tmpMessage=typeof pMessage!=='undefined'?pMessage:'Time Measurement';let tmpEndTime=+new Date();let tmpOperationTime=tmpEndTime-pTimeStamp;this.info(tmpMessage+' ('+tmpOperationTime+'ms)');}}module.exports=InformaryLog;},{}],14:[function(require,module,exports){/**
* @license MIT
* @author <steven@velozo.com>
*/let libObjectDiff=require('deep-object-diff');let libCacheTraxx=require('cachetrax');/**
* Informary browser sync library
*
* @class Informary
*/class Informary{constructor(pSettings,pContext,pContextGUID){this._Dependencies={};this._Dependencies.jqueryLibrary=require('jquery');// Adding a container for non-html state to be stored in, which will be marshalled into and out of the passed in FormData.
this._NonHTMLState={};this._NonHTMLStateProperty="__InformaryNonHTMLState";this._Settings=typeof pSettings==='object'?pSettings:{// The form we are dealing with (this is a hash set on the form itself)
...
this._Context=pContext?pContext.toString():'InformaryDefaultContext';this._ContextGUID=pContextGUID?pContextGUID.toString():'0x000000001';}/******************************************************
	 ...
	 */setStorageProvider(pStorageProvider){this._LocalStorage=pStorageProvider;}checkStorageProvider(){// When running in a browser, this likely won't be set.  If it isn't,
if(!this._LocalStorage){this._LocalStorage=window.localStorage;if(!this._LocalStorage){const cache={};this._LocalStorage={setItem:(key,value)=>{cache[key]=value;},getItem:key=>{return cache[key];},removeItem:key=>{delete cache[key];}};}}}getIndexKey(pValueType){return"Informary_Index_User[".concat(this._Settings.User.toString(),"]_ValueType[").concat(pValueType,"]");}getStorageKey(pValueType){return"Informary_Data_User[".concat(this._Settings.User.toString(),"]_ValueType[").concat(pValueType,"]_Context[").concat(this._Context,"]_ContextGUID[").concat(this._ContextGUID,"]");}// Read the whole index
readIndex(pValueType){this.checkStorageProvider();let tmpIndex=false;let tmpData=this._LocalStorage.getItem(this.getIndexKey(pValueType));if(tmpData){try{tmpIndex=JSON.parse(tmpData);}catch(pError){this.log.error("Error parsing local storage index key [".concat(this.getIndexKey(pValueType),"]"));}}if(!tmpIndex){tmpIndex={IndexCreateTime:Date.now(),IndexUser:this._Settings.User};}tmpIndex.IndexLastReadTime=Date.now();return tmpIndex;}// Read just the record key for the index
readIndexValue(pValueType){let tmpIndex=this.readIndex(pValueType);let tmpIndexKeyValue=tmpIndex[this.getStorageKey(pValueType)];// Rather than return undefined, return false if it's a miss
return tmpIndexKeyValue?tmpIndexKeyValue:false;}// Touch the index for a value type
touchIndex(pValueType){this.checkStorageProvider();let tmpIndex=this.readIndex(pValueType);let tmpKey=this.getStorageKey(pValueType);tmpIndex[tmpKey]={Time:Date.now(),ValueType:pValueType,User:this._Settings.User,Context:this._Context,ContextGUID:this._ContextGUID};// This relies on the readIndex above to initialize the localstorage provider
this._LocalStorage.setItem(this.getIndexKey(pValueType),JSON.stringify(tmpIndex));}readData(pValueType){// Check that the storage provider is initialized
this.checkStorageProvider();let tmpData=this._LocalStorage.getItem(this.getStorageKey(pValueType));if(tmpData){try{tmpData=JSON.parse(tmpData);}catch(pError){this.log.error("Error parsing local storage key [".concat(this.getStorageKey(pValueType),"]"));tmpData=false;}}else{tmpData=false;}return tmpData;}writeData(pValueType,pData){// Check that the storage provider is initialized
this.checkStorageProvider();// Touch the index with a timestamp for the value
this.touchIndex(pValueType);// set the actual item
this._LocalStorage.setItem(this.getStorageKey(pValueType),JSON.stringify(pData));}deleteData(pValueType){// Check that the storage provider is initialized
...
this._LocalStorage.removeItem(this.getStorageKey(pValueType));}/*
	 ...
	 ******************************************************/getValueAtAddress(pObject,pAddress){// Make sure pObject is an object
if(!typeof pObject==='object')return false;// Make sure pAddress is a string
if(!typeof pAddress==='string')return false;let tmpSeparatorIndex=pAddress.indexOf('.');if(tmpSeparatorIndex===-1){// Now is the time to return the value in the address
return pObject[pAddress];}else{let tmpSubObjectName=pAddress.substring(0,tmpSeparatorIndex);let tmpNewAddress=pAddress.substring(tmpSeparatorIndex+1);// If there is an object property already named for the sub object, but it isn't an object
...
pObject[tmpSubObjectName]={};return this.getValueAtAddress(pObject[tmpSubObjectName],tmpNewAddress);}}}setValueAtAddress(pObject,pAddress,pValue){// Make sure pObject is an object
if(!typeof pObject==='object')return false;// Make sure pAddress is a string
if(!typeof pAddress==='string')return false;let tmpSeparatorIndex=pAddress.indexOf('.');if(tmpSeparatorIndex===-1){// Now is the time to set the value in the object
pObject[pAddress]=pValue;return true;}else{let tmpSubObjectName=pAddress.substring(0,tmpSeparatorIndex);let tmpNewAddress=pAddress.substring(tmpSeparatorIndex+1);// If there is an object property already named for the sub object, but it isn't an object
// then the system can't set the value in there.  Error and abort!
...
pObject[tmpSubObjectName]={};return this.setValueAtAddress(pObject[tmpSubObjectName],tmpNewAddress,pValue);}}}setValueAtAddressInContainer(pRecordObject,pFormContainerAddress,pFormContainerIndex,pFormValueAddress,pFormValue){// First see if there *is* a container object
...
tmpContainerObject=[];this.setValueAtAddress(pRecordObject,pFormContainerAddress,tmpContainerObject);}for(let i=0;tmpContainerObject.length+i<=tmpFormContainerIndex+1;i++){// Add objects to this container until it has enough
...
storeSourceData(pData){return this.writeData('Source',pData);}// Write out recovery data
storeRecoveryData(fCallback){let tmpCallback=typeof fCallback=='function'?fCallback:()=>{};let tmpRecoveryData={};this.marshalFormToData(tmpRecoveryData,()=>{this._RecoveryDocumentState=tmpRecoveryData;return tmpCallback(this.writeData('Recovery',this._RecoveryDocumentState));});}snapshotData(){let tmpNewUndoKey=Date.now().toString();// Check to see if there are any changes to the data to be stored
let tmpOldRecoveryState=JSON.stringify(this._RecoveryDocumentState);this.storeRecoveryData(()=>{if(tmpOldRecoveryState!=JSON.stringify(this._RecoveryDocumentState)){if(this._Settings.DebugLog){this.log.debug("Creating recovery snapshot at [".concat(tmpNewUndoKey,"]..."));}// Destroy all data in the redo ring, because this new snapshot invalidates it.
while(this._RedoKeys.length>0){let tmpRedoKey=this._RedoKeys.pop();this._RedoBuffer.expire(tmpRedoKey);}this._UndoKeys.push(tmpNewUndoKey);this._UndoBuffer.put(this._RecoveryDocumentState,tmpNewUndoKey);}else{if(this._Settings.DebugLog){this.log.debug("Skipped creating recovery snapshot at [".concat(tmpNewUndoKey,"] because there were no changes to the recovery state..."));}}});}snapshotDataInitial(){let tmpNewUndoKey=Date.now().toString();if(this._UndoKeys.length>0){this.log.info("Skipping creation of initial snapshot because one already exists.");return false;}this.storeRecoveryData(()=>{if(this._Settings.DebugLog){this.log.debug("Creating recovery snapshot at [".concat(tmpNewUndoKey,"]..."));}this._UndoKeys.push(tmpNewUndoKey);this._UndoBuffer.put(this._RecoveryDocumentState,tmpNewUndoKey);});return true;}undoSnapshotCount(){// The initial snapshot is special to prevent confusing conditions when form data hasn't been edited.
...
let tmpCurrentFormData={};this.marshalFormToData(tmpCurrentFormData,()=>{this.marshalDataToForm(tmpSnapshotData,()=>{this._RecoveryDocumentState=tmpSnapshotData;this.log.info("Informary reapplied snapshot ID ".concat(tmpSnapshotKey));if(JSON.stringify(tmpCurrentFormData)==JSON.stringify(tmpSnapshotData)&&this._RedoKeys.length>0){// If the old form data matches the applied data, roll back farther
return this.reapplyNextRevertedSnapshot(tmpCallback);}return tmpCallback(true);});});}return tmpCallback(false);}clearRecoveryData(){return this.deleteData('Recovery');}readRecoveryData(){return this.readData('Recovery');}restoreRecoveryScenarioData(fCallback){let tmpCallback=typeof fCallback=='function'?fCallback:()=>{};let tmpRecoveryScenarioData=this.readRecoveryScenario();if(tmpRecoveryScenarioData&&tmpRecoveryScenarioData.ExistingRecovery){this.marshalDataToForm(tmpRecoveryScenarioData.ExistingRecovery,()=>{this.clearRecoveryScenarioData();// Store a new recovery data
//this.storeSourceData();
return tmpCallback(true);});}return tmpCallback(false);}clearRecoveryScenarioData(){return this.deleteData('RecoveryScenario');}storeRecoveryScenarioData(pRecoveryScenarioData){return this.writeData('RecoveryScenario',pRecoveryScenarioData);}readRecoveryScenario(){return this.readData('RecoveryScenario');}// Checks if there is a recovery record, and detailed data about what it might be
checkRecoveryState(pSourceData){let tmpRecoveryData={NewSource:pSourceData,ExistingSource:this.readData('Source'),ExistingRecovery:this.readData('Recovery')};if(!tmpRecoveryData.ExistingSource||!tmpRecoveryData.ExistingRecovery){// There is either no source or no read data, so we are not in a recovery state
return false;}else{// Now check the differences
let tmpRecoveryDifferences=libObjectDiff.detailedDiff(tmpRecoveryData.ExistingSource,tmpRecoveryData.ExistingRecovery);if(JSON.stringify(tmpRecoveryDifferences)==JSON.stringify(libObjectDiff.detailedDiff({},{}))){// No differences -- we're good for now
...
let tmpFirstRedoIndexWithDifferences=this._RedoKeys.length-1;for(let i=this._RedoKeys.length-1;i>=0;i--){let tmpRedoComparisonJSON=JSON.stringify(this._RedoBuffer.read(this._RedoKeys[i]));if(tmpRedoComparisonJSON!=tmpCurrentStateDataJSON){tmpFirstRedoIndexWithDifferences=i;// Once we have found a set of redo data that doesn't match, we don't want to keep looking
break;}}tmpCurrentRedoObject=this._RedoBuffer.read(this._RedoKeys[tmpFirstRedoIndexWithDifferences]);}let tmpComparisonData={UndoDelta:libObjectDiff.detailedDiff(tmpCurrentStateData,tmpCurrentUndoObject),UndoGUIDDelta:{Added:[],Deleted:[]},RedoDelta:libObjectDiff.detailedDiff(tmpCurrentStateData,tmpCurrentRedoObject),RedoGUIDDelta:{Added:[],Deleted:[]}};// Perform GUID diff operations
// Get all GUID values from the form
let tmpCurrentGUIDElements=[];let tmpCurrentDataIndex=0;if(tmpCurrentStateData.hasOwnProperty('__GUID')){tmpCurrentGUIDElements=Object.keys(tmpCurrentStateData.__GUID).sort();}// Get the deltas for undo data
let tmpUndoGUIDElements=[];if(tmpCurrentUndoObject.hasOwnProperty('__GUID')){tmpUndoGUIDElements=Object.keys(tmpCurrentUndoObject.__GUID).sort();}let tmpUndoDataIndex=0;let tmpUndoDataMaxIndex=tmpUndoGUIDElements.length-1;for(tmpCurrentDataIndex=0;tmpCurrentDataIndex<tmpCurrentGUIDElements.length;tmpCurrentDataIndex++){while(tmpUndoDataIndex<=tmpUndoDataMaxIndex&&tmpUndoGUIDElements[tmpUndoDataIndex]!=tmpCurrentGUIDElements[tmpCurrentDataIndex]){// Check to see if the string in the Undo keys is less than the string in the current form element.
...
for(let i=tmpUndoDataIndex;i<=tmpUndoDataMaxIndex;i++){tmpComparisonData.UndoGUIDDelta.Added.push(tmpUndoGUIDElements[i]);}// Get the deltas for Redo data
...
this._NonHTMLState=pRecordObject[this._NonHTMLStateProperty];}}let tmpRecordObjectKeys=Object.keys(pRecordObject);tmpRecordObjectKeys.forEach(pKey=>{let tmpRecord=pRecordObject[pKey];let tmpPropertyAddress=tmpParentPropertyAddress.length>0?"".concat(pParentPropertyAddress,".").concat(pKey):pKey;if(this._Settings.DebugLog){this.log.debug("Informary Data->Form parent address [".concat(tmpParentPropertyAddressString,"] parsing property [").concat(tmpPropertyAddress,"]"));}switch(typeof tmpRecord){// If it's an object, check if we should be marshaling the whole value in or recursing.
case'object':// Check to see if it's an array, as we will put it into the extended object.
if(Array.isArray(tmpRecord)){for(let i=0;i<tmpRecord.length;i++){// The undefined is in the Property Address because this is an array element, and needs to be put in the array.
this.marshalDataToForm(tmpRecord[i],()=>{},undefined,tmpPropertyAddress,i.toString());}}else{// We've switched this to synchronous for safe browser mode
// Leaving an empty callback in there in case we decide to switch back.
return this.marshalDataToForm(tmpRecord,()=>{},tmpPropertyAddress,tmpContainerPropertyAddress,tmpContainerPropertyIndex.toString());}break;// Ignore undefined properties
case'undefined':break;// Otherwise marshal it into the form
default:let tmpFormElement=[];if(tmpContainerPropertyAddress&&tmpContainerPropertyIndex){// This is an array element
...
if(this._Dependencies.jquery(tmpFormElement)[0].tagName==='TEXTAREA'){this._Dependencies.jquery(tmpFormElement)[0].textContent=tmpRecord;// set the correct option to 'selected' for select tags
}else if(this._Dependencies.jquery(tmpFormElement)[0].tagName==='SELECT'){this._Dependencies.jquery("select[data-i-form=\"".concat(this._Settings.Form,"\"][data-i-datum=\"").concat(tmpPropertyAddress,"\"] option[value=\"").concat(tmpRecord,"\"]")).prop('selected',true);// otherwise just set the value for input
}else{this._Dependencies.jquery(tmpFormElement).val(tmpRecord);}// Check if this is a GUID value and set the data-i-guid property in it
var tmpGUIDAttribute=this._Dependencies.jquery(this).attr('data-i-guid');// For some browsers, `attr` is undefined; for others,
...
if(typeof pRecordObject!=='object'){this.log.error('Invalid record object passed in!  Informary needs a Javascript object to put values into.');return fCallback('Invalid record object passed in!  Informary needs a Javascript object to put values into.');}let tmpFormValueElements=this._Dependencies.jquery("\n\t\t\t\tinput[data-i-form=".concat(this._Settings.Form,"],\n\t\t\t\tselect[data-i-form=").concat(this._Settings.Form,"],\n\t\t\t\ttextarea[data-i-form=").concat(this._Settings.Form,"]\n\t\t\t"));let tmpUnknownValueIndex=0;// For any state that the form doesn't want to store in html elements, but still be merged into the informary record object
pRecordObject[this._NonHTMLStateProperty]=this._NonHTMLState;this._Dependencies.jquery.each(tmpFormValueElements,(pRecordIndex,pRecordAddress)=>{let tmpFormValueAddress=this._Dependencies.jquery(pRecordAddress).attr('data-i-datum');let tmpFormContainerAddress=this._Dependencies.jquery(pRecordAddress).attr('data-i-container');let tmpFormContainerIndex=this._Dependencies.jquery(pRecordAddress).attr('data-i-index');let tmpFormContainerGUID=this._Dependencies.jquery(pRecordAddress).attr('data-i-guid');let tmpFormValue;// check to see which element type this is before trying to collect the value
if(this._Dependencies.jquery(pRecordAddress).tagName==='TEXTAREA'){tmpFormValue=this._Dependencies.jquery(pRecordAddress).textContent;}else{tmpFormValue=this._Dependencies.jquery(pRecordAddress).val();}// If the value is non existant, set it to null
if(typeof tmpFormValue==='undefined'){tmpFormValue=null;}if(typeof tmpFormValueAddress==='undefined'){tmpFormValueAddress='__ERROR.UnsetDatum.'+tmpUnknownValueIndex;tmpUnknownValueIndex++;}if(tmpFormContainerGUID){let tmpGUIDValueAddress='__GUID.'+tmpFormContainerGUID;this.setValueAtAddress(pRecordObject,tmpGUIDValueAddress,tmpFormContainerGUID);}if(tmpFormContainerAddress&&tmpFormContainerIndex){this.setValueAtAddressInContainer(pRecordObject,tmpFormContainerAddress,tmpFormContainerIndex,tmpFormValueAddress,tmpFormValue);}else{this.setValueAtAddress(pRecordObject,tmpFormValueAddress,tmpFormValue);}});return fCallback();}};module.exports=Informary;},{"./Informary-Log.js":13,"cachetrax":1,"deep-object-diff":8,"jquery":11}]},{},[12])(12);});